...
CCostume_Uniform::CCostume_Uniform(std::string GID)
    : CCostumeBase("media/graphic/character/komuk/costume/uniform/komuk.png",
                   ...
                   COF(0.7, 0.96, 0.05, 0.0), GID) {
  detail = CTextDrawer::Text(
      "最初に来ていた制服。最も基本的な衣装であり、\n全ての耐性が平均的。\n連射"
      "は誘導魔法弾。\n最も近い敵を探して攻撃する。\nしかし壁に当たると消えてし"
      "まう。\n強射は高出力魔法弾。\n周囲の環境の影響を受けずに\nまっすぐ飛んで"
      "高めのダメージを与える。",
      Constant::Costume_detail_pos, 0xFFFFFF, 0x000000, 0);
}
...
void CCostume_Uniform::WeakShot(CAttribute baseATK, CVector position,
  ...
  // p_player->RegisterPower(std::make_shared<CPower_Line>(position +
  // CVector(angle) * 32, CVector(angle, 128.0), 64, 30, 1));
}
