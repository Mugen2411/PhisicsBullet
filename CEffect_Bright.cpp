...
CEffect_Bright::CEffect_Bright() : brightLevel(0) {
...
void CEffect_Bright::Render()const {
	...
	for (auto& i : list) {
		...
		SetDrawBlendMode(DX_BLENDMODE_ADD, i.power * 0xFF);
		DrawRotaGraph(i.position.x + CAnchor::getIns().getAnchorX(), i.position.y + CAnchor::getIns().getAnchorY(), i.radius / 256.0, 0, brightGraph, TRUE);
	}
	SetDrawScreen(shadowBuffer);
