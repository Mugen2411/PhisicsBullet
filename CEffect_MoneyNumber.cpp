...
void CEffect_MoneyNumber::Render() const {
  ...
  int n = num_;
  int x = (int)position_.x + l * 5;
  for (int i = 0; i < l; i++) {
    CImageManager::GetIns()
        .Find("effect_number3")
        ->DrawRota(x, (int)position_.y, 0.0, 1.0, Constant::kPriorityNumber, n % 10);
    x -= 10;
    n /= 10;
  }
  CImageManager::GetIns()
      .Find("effect_number3")
      ->DrawRota(x, (int)position_.y, 0.0, 1.0, Constant::kPriorityNumber, 10);
}
