...
CField_Cave::CField_Cave(std::string gid, CVector position)
	:CField(gid, position, CVector(32.0, 32.0), COF().setFrictionCF(0.5), 0), animCount(0)
{
...
{
	CImageManager::getIns().find("Field_Cave")->DrawRota(Position.x, Position.y, 0.0, 1.0, Constant::priority_field, decoration);
}
