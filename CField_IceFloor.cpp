	...
	:CField(f, "field_icefloor", position, CVector(32.0, 32.0), 0.3, 0, 1.0)
{
	if (CImageManager::getIns().find("field_icefloor") == nullptr) {
		CImageManager::getIns().set("field_icefloor", std::make_shared<CImage>("media/graphic/mapchip/test_ice.png"));
	}
}
...
{
	CImageManager::getIns().find("field_icefloor")->DrawRota(Position.x, Position.y, 0.0, 1.0);
