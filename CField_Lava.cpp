...
CField_Lava::CField_Lava(std::string gid, CVector position, double temperature)
	:CField(gid, position, CVector(32.0, 32.0), COF().setFrictionCF(0.6), temperature, CAttribute(0.0))
{
}
...
{
	if (Temperature < 0)CImageManager::getIns().find("Field_Lava")->DrawRota(Position.x, Position.y, 0.0, 1.0, Constant::priority_field, 1);
	else {
		CEffect_Bright::getIns().Register(CEffect_Bright::BrightDesc(Position, 512 - GetRand(16), 64));
		CImageManager::getIns().find("Field_Lava")->DrawRota(Position.x, Position.y, 0.0, 1.0, Constant::priority_field, 0);
	}
}
