...
bool CMover::OnWall(CField* f, double WallReflectionCF) {
  ...
  CVector diff = velocity_ + acceleration_;
  int cnt = (int)ceil(diff.GetLength() / 32.0);
  diff = diff / (double)cnt;
  auto setU = [&]() {
  ...
  for (int i = 1; i <= cnt; i++) {
    CVector nextPosition = position_ + diff * i;
    ...
    double _max =
               (nextPosition.x_ + size_) - (WallPosition.x_ - WallSize.x_ / 2),
           _may =
               (nextPosition.y_ + size_) - (WallPosition.y_ - WallSize.y_ / 2);
    double _nax =
               (WallPosition.x_ + WallSize.x_ / 2) - (nextPosition.x_ - size_),
           _nay =
               (WallPosition.y_ + WallSize.y_ / 2) - (nextPosition.y_ - size_);
    ...
    if (may > 0 && nay > 0) {
      if (_nax > 0 &&
          (position_.x_ - size_) > (WallPosition.x_ - WallSize.x_ / 2)) {
        //左にある
        L = true;
        setL();
      }
      if (_max > 0 &&
          (WallPosition.x_ + WallSize.x_ / 2) > (position_.x_ + size_)) {
        //右にある
        R = true;
        setR();
      }
    ...
    if (max > 0 && nax > 0) {
      if (_nay > 0 &&
          (position_.y_ - size_) > (WallPosition.y_ - WallSize.y_ / 2)) {
        //上にある
        U = true;
        setU();
      }
      if (_may > 0 &&
          (WallPosition.y_ + WallSize.y_ / 2) > (position_.y_ + size_)) {
        //下にある
        D = true;
        setD();
      }
    ...
    if (R || L) {
      ...
      acceleration_.x_ *= -cofs_.ReflectCF * WallReflectionCF;
      //acceleration_.x_ = 0;
    ...
    if (U || D) {
      ...
      acceleration_.y_ *= -cofs_.ReflectCF * WallReflectionCF;
      //acceleration_.y_ = 0;
