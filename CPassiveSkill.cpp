...
CTextDrawer::Text CPassiveSkill::getText(int index)
{
	...
	if (index <= ATK_WIND) {
		ret.text = std::string("与える") + attr[index % 7] + std::string("属性ダメージを") + std::to_string(has[index] * attrATKperLEVEL) +
			std::string("%→") + std::to_string((has[index] + 1) * attrATKperLEVEL) + std::string("%増加する。");
		return ret;
	}
	if (index <= DEF_WIND) {
		ret.text = std::string("受ける") + attr[index % 7] + std::string("属性ダメージを") + std::to_string(has[index] * attrDEFperLEVEL) +
			std::string("%→") + std::to_string((has[index] + 1) * attrDEFperLEVEL) + std::string("%軽減する。");
		return ret;
	}
	if (index == ATK_ALL) {
		ret.text = std::string("与える") + std::string("全てのダメージを") + std::to_string(has[index] * allATKperLEVEL) +
			std::string("%→") + std::to_string((has[index] + 1) * allATKperLEVEL) + std::string("%増加する。");
		return ret;
	}
	if (index == DEF_ALL) {
		ret.text = std::string("受ける") + std::string("全てのダメージを") + std::to_string(has[index] * allDEFperLEVEL) +
			std::string("%→") + std::to_string((has[index] + 1) * allDEFperLEVEL) + std::string("%軽減する。");
		return ret;
	}
	if (index == HEAL) {
		ret.text = std::string("HPを毎秒") + floating_to_string(has[index] * healPerLEVEL) +
			std::string("%→") + floating_to_string((has[index] + 1) * healPerLEVEL) + std::string("%回復する。");
		return ret;
	}
	if (index == MONEY) {
		ret.text = std::string("獲得する全てのコインを") + std::to_string(has[index] * moneyPerLEVEL) +
			std::string("%→") + std::to_string((has[index] + 1) * moneyPerLEVEL) + std::string("%増加する。");
		return ret;
	}
	if (index == CHARGE) {
		ret.text = std::string("射撃のチャージ速度を") + std::to_string(has[index] * chargePerLEVEL) +
