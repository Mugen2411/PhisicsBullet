...
CPower_Line::CPower_Line(CVector position, CVector direction, double width,
    ...
    : CPower(position, power, duration), direction_(direction), width_(width) {
...
void CPower_Line::ApplyForceToMover(CMover* m) {
  if (isHit(m->GetPosition(), m->GetSize()))
    m->ApplyAirForce(force_direction_ * power_ /
...
void CPower_Line::Render() const {
  // CImageManager::GetIns().Find("effect_wind")->DrawModi(corner_[0].x_,
  // corner_[0].y_, corner_[1].x_, corner_[1].y_, 	corner_[2].x_, corner_[2].y_,
  //corner_[3].x_, corner_[3].y_, Constant::priority_effect, 0);
  for (int i = 0; i < 3; i++) {
