...
Scene_Title::Scene_Title(SceneManager* ScnMng) :Scene_Abstract(ScnMng), CMS(6, 0), currentStage(0) {
	input = CControllerFactory::getIns().getController();
	menuText[0] = CTextDrawer::Text("Start", CVector(340, 200), 0xFFFFFF, 0x007F7F, 1);
	menuText[1] = CTextDrawer::Text("Endless", CVector(340, 240), 0xFFFFFF, 0x000000, 1);
	menuText[2] = CTextDrawer::Text("Upgrade", CVector(340, 280), 0xFFFFFF, 0x7F7F00, 1);
	menuText[3] = CTextDrawer::Text("Option", CVector(340, 320), 0xFFFFFF, 0x7F7F7F, 1);
	menuText[4] = CTextDrawer::Text("Manual", CVector(340, 360), 0xFFFFFF, 0x007F00, 1);
	menuText[5] = CTextDrawer::Text("Quit", CVector(340, 400), 0xFFFFFF, 0x7F0000, 1);
	underText[0] = CTextDrawer::Text("ADキーでステージを選択しSPACEキーでゲームを開始します。", CVector(36, 480 - 22), 0xFFFFFF, 0x000000, 0);
	underText[1] = CTextDrawer::Text("エンドレスモードでゲームを開始します。", CVector(36, 480 - 22), 0xFFFFFF, 0x000000, 0);
	underText[2] = CTextDrawer::Text("ゲーム内通貨を使用して自機のレベルを上げます。", CVector(36, 480 - 22), 0xFFFFFF, 0x000000, 0);
	underText[3] = CTextDrawer::Text("各種設定を変更します。", CVector(36, 480 - 22), 0xFFFFFF, 0x000000, 0);
	underText[4] = CTextDrawer::Text("操作方法やルールの説明を見ます。(メモ帳を開きます)", CVector(36, 480 - 22), 0xFFFFFF, 0x000000, 0);
	underText[5] = CTextDrawer::Text("ゲームを終了します。", CVector(36, 480 - 22), 0xFFFFFF, 0x000000, 0);
...
void Scene_Title::Update() {
	...
	//menuText[1].text = std::string("Endless Start→") + std::to_string(currentStage + 1);
